...
title: Context API
summary: >-
  La API de Contexto nos permite manejar el estado de nuestra aplicación sin
  necesidad de utilizar Redux.
category: react
tags:
  options:
    - react
...
relatedPosts:
  ...
  postTwo: content/posts/react/conceptos-avanzados.mdx
      ...
      <div>
        {NewsContext.news.length
          ? NewsContext.news.map((news) => (
              <div key={news.id} onClick={() => NewsContext.getDescription(news.id)}>
                {news.title}
              </div>
            ))
          : null}
...
Puedes continuar leyendo el siguiente capítulo sobre [React Router](/blog/react/router ""), volver al capítulo anterior de [Conceptos avanzados](/blog/react/conceptos-avanzados ""), o volver al [Índice](/blog/react/indice "").
